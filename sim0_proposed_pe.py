...
def run_proposed_sysid(n=2, T=30, dt=0.001, use_shift=False, shift_time=15.0):
    ...
    for i in tqdm(range(N-1), desc=f"Proposed n={n}"):
        ...
        # Add Persistent Excitation (sum of sinusoids with different frequencies)
        pe_signal = np.zeros(n)
        for d in range(n):
            pe_signal[d] = 0.5 * np.sin((10 + d*3) * t) + 0.3 * np.cos((25 + d*5) * t)
        u_vec = u_vec + pe_signal
...
if __name__ == "__main__":
    for n, label in [(2, 'n2'), (6, 'n6')]:
        for use_shift, suf in [(False, 'normal'), (True, 'shift')]:
            ...
            base = f"results/sysid_pe/{label}/{suf}"
